...
- 📖 **睡眠记录与统计**：
  - 睡眠记录由联动自动写入（见下方「睡眠时间管理联动」），无需手动打卡；
  - 「睡眠记录」页保留手动表单与逐日列表，用于修正联动数据，同一日期再次保存会更新原记录；
  - 逐日保存入睡日期、入睡/起床时间、主睡眠分钟数和可选午睡分钟数。
...
- 🔗 **睡眠时间管理联动**：
  - 推荐时间卡点「设闹钟」时自动写入今晚睡眠计划两端：入睡 = 按下按钮时刻 + 入睡潜伏期（与推荐计算口径一致），起床 = 闹钟响铃时间；
  - 按「睡眠日」归属归档（凌晨设定归当天、其余归次日），入睡与起床始终落在同一条记录；
  - 小睡预设设定后自动记录午睡时长；两端齐备即自动结算时长并进入「睡眠分析」统计；
  - 联动发生在设定时刻，半成品记录不计入统计，事后可在记录页手动修正（最后操作生效）。
- ⏱️ **高效小睡模式**：
...
- 📊 **可视化统计（睡眠分析页）**：
  - **时长趋势柱状图**：7/14/30 天窗口内每日主睡眠柱 + 睡眠目标虚线，达标/接近/缺口大三档柱色，一眼看出趋势与欠账；
  - **作息带状图**：每天一条入睡→起床横带，时间轴跨午夜对齐，作息漂移一目了然；
  - **睡眠热力图**：格子日历按周分行，按达标四档着色（达标/接近/缺口大/无记录），像打卡日历一样回顾整段时间；
  - 三图共享统计窗口切换与点选——点选任意一天高亮并显示该日详情（日期、入睡、起床、主睡眠、午睡）；
  - 全部手写 Compose Canvas 绘制，玻璃卡片风格与全局一致，深浅色主题均可读。
- 🌅 **起床窗口副标签**：
...
- **[SleepCycle-v1.11.0.apk](../../releases/latest)**
