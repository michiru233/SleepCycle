...
- 🛏️ **一键打卡与睡眠记录**：
  - 首页顶端常驻醒目的毛玻璃打卡卡片，支持一键记录「我睡了」和「我醒了」，自动捕获系统时刻；
  - 智能跨午夜归档与卧床时长自动结算，记录实时联动「睡眠记录」列表与「睡眠分析」图表；
...
- 🧭 **清晰导航与专注首页**：
  - 首页聚焦睡眠计算、时间/入睡潜伏期设置、推荐结果与系统闹钟闭环；记录、分析、时间型与光照、小睡、知识和设置/关于通过可见导航抽屉进入。
  - 顶栏菜单按钮与左边缘手势均可打开导航抽屉，选择入口后自动关闭；返回键会先关闭已展开的抽屉。
- 🔄 **应用内检查更新**：
  - 在「设置/关于」中连接 GitHub Releases API 检索最新版本并比对语义化版本。
...
- 🎨 **液态玻璃 (Liquid Glass) 界面**：
  ...
  - 顶栏一键切换深色/浅色模式（月亮/太阳图标），手动选择持久化保存，从未切换时跟随系统。
...
- **[SleepCycle-v1.9.4.apk](../../releases/latest)**
